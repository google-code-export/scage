...
import su.msk.dunno.screens.support.tracer.{Tracer, State}
...
class WeaponTracer extends Tracer[FieldObject] (
  ...
  def drawWeapon = {
    for(x <- 0 to N_x-1) {
      for(y <- 0 to N_y-1) {
        println(x+":"+y+" - "+coord_matrix(x)(y).length)
        if(coord_matrix(x)(y).length > 0) {
          println(coord_matrix(x)(y).last.getCoord)
          coord_matrix(x)(y).last.draw
        }
      }
    }
    System.exit(0)
