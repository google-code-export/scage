...
import su.msk.dunno.scage2.support.{ScageProperties, Color, Vec}
...
class Renderer(screen:Screen) extends Handler(screen:Screen) {
  ...
  val center = Vec(Renderer.width/2, Renderer.height/2)
  private var central_coord = () => Vec(Renderer.width/2, Renderer.height/2)
