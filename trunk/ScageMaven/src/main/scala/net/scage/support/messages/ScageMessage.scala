...
class ScageMessage(
  ...
  def xmlOrDefault(message_code:String, parameters:Any*):String = {
    if(xmlmh.xml_messages.contains(message_code)) mergeMessage(xmlmh.xml_messages(message_code), parameters.tail:_*)
    else {
      if(parameters.size > 0) {
        log.info("default value for string code "+message_code+" is "+{
          if("" == parameters.head) "empty string" else parameters.head
        })
        xmlmh.xml_messages += (message_code -> parameters.head.toString)
