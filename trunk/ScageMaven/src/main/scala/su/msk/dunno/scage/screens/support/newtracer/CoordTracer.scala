...
import su.msk.dunno.scage.single.support.Vec._

trait CoordTrace extends Trace {
  private val _coord:Vec = Vec(-1,-1)
  private[newtracer] def coord_=(new_coord:Vec) {_coord is new_coord}
  def coord:Vec = _coord
}
